...
pub fn build(b: *std.Build) !void {
    ...
    const static_lib: ?*Step.Compile = lib: {
        ...
        if (target.result.os.tag == .windows) {
            c_api_module.linkSystemLibrary("ws2_32", .{});
            c_api_module.linkSystemLibrary("mswsock", .{});
        }
    ...
    const test_exe: *Step.Compile = test_exe: {
        ...
        switch (target.result.os.tag) {
            .linux, .macos => test_module.link_libc = true,
            else => {},
        }
...
fn buildBenchmarks(
    ...
    const io = b.graph.io;
    var dir = try b.build_root.handle.openDir(io, "src/bench", .{ .iterate = true });
    defer dir.close(io);
...
fn manPages(b: *std.Build) ![]const *Step {
    ...
    const io = b.graph.io;
    var dir = try b.build_root.handle.openDir(io, "docs", .{ .iterate = true });
    defer dir.close(io);
