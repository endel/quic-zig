...
pub fn run(comptime n_senders: comptime_int, comptime n_receivers: comptime_int, io: Io) !void {
    ...
    for (&receivers, 0..) |*r, i| {
        const addr = try xev.net.Address.parseIp4("127.0.0.1", @as(u16, @intCast(base_port + i)));
    ...
    for (&senders, 0..) |*s, i| {
        const addr = try xev.net.Address.parseIp4(
            "127.0.0.1",
