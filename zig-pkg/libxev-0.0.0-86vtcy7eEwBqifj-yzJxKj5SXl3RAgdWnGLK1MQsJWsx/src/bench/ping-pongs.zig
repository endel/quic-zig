...
// In Zig 0.16, `std.heap.MemoryPool(T)` returns the unmanaged variant
// (different API: pool.create(allocator) instead of pool.create()). This
// bench was written against the managed API; opt into the renamed
// `ExtraManaged` to keep the call sites unchanged.
const BufferPool = std.heap.memory_pool.ExtraManaged([4096]u8, .{});
const CompletionPool = std.heap.memory_pool.ExtraManaged(xev.Completion, .{});
const TCPPool = std.heap.memory_pool.ExtraManaged(xev.TCP, .{});
...
const Client = struct {
    ...
    pub fn start(self: *Client) !void {
        const addr = try xev.net.Address.parseIp4("127.0.0.1", 3131);
