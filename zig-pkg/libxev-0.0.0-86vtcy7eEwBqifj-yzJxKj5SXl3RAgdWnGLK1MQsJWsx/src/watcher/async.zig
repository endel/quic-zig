        ...
        pub fn init() !Self {
            return .{
                .fd = switch (builtin.os.tag) {
                    ...
                    // Linux still has eventfd via the local syscall shim.
                    else => try posix.eventfd(
                        0,
...
/// A tiny spin lock used to guard the two-field AsyncIOCP state. Zig 0.16
/// removed `std.Thread.Mutex` in favor of `std.Io.Mutex`, which requires an
/// `Io` parameter we don't thread through libxev's proactor API. The
/// critical section here is a pair of field writes, so a spin lock is
/// fine — contention is extremely rare (one `wait` call racing with one
/// cross-thread `notify`).
