...
fn FileStream(comptime xev: type) type {
    return struct {
        ...
        test "queuePWrite" {
            ...
            // windows: std.Io.File is not opened with OVERLAPPED flag.
            ...
            const f = try std.Io.Dir.cwd().createFile(std.Options.debug_io, path, .{
                .read = true,
                .truncate = true,
            });
            defer f.close();
            ...
            const f2 = try std.Io.Dir.cwd().openFile(std.Options.debug_io, path, .{});
            defer f2.close();
