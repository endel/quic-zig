...
const xev_net = @import("../net.zig");
const builtin = @import("builtin");
const assert = std.debug.assert;
const posix = @import("../syscall.zig");
...
/// Local replacement for std.posix.KEventError which was removed in Zig 0.16.
/// Mirrors the error set returned by the 0.15.x `posix.kevent`/`kevent64` wrappers.
...
/// Re-export shared catch-all posix error set from the syscall helper.
pub const PosixError = posix.Error;
...
pub const AcceptError = KEventError || PosixError || error{
    Canceled,
    Unexpected,
};

pub const ConnectError = KEventError || PosixError || error{
    Canceled,
    Unexpected,
};

pub const ReadError = KEventError ||
    PosixError ||
    error{
        EOF,
        Canceled,
...
pub const WriteError = KEventError ||
    PosixError ||
    error{
        Canceled,
        PermissionDenied,
...
pub const ShutdownError = PosixError || error{
    Canceled,
    Unexpected,
};
...
fn kevent_syscall(
    ...
    // Normal Kevent? Use the shared posix wrapper and narrow its broad
    // `syscall.Error` set down to the subset kevent is actually documented
    // to return. Everything else is a bug in our event setup, not a
    // runtime condition callers should handle.
    if (Kevent == posix.Kevent) return posix.kevent(
        kq,
        changelist,
        eventlist,
        timeout,
