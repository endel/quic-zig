    ...
    pub fn perform(self: *Completion) Result {
        return switch (self.op) {
            ...
            .accept => |*v| {
                ...
                var flags: u32 = 0;
                const result = windows.ws2_32.WSAGetOverlappedResult(asSocket(v.socket), &self.overlapped, &bytes_transferred, .FALSE, &flags);
            ...
            .read => |*v| {
                ...
                const result = windows.kernel32.GetOverlappedResult(v.fd, &self.overlapped, &bytes_transferred, .FALSE);
                if (result == .FALSE) {
            ...
            .send => |*v| {
                ...
                const result = windows.ws2_32.WSAGetOverlappedResult(asSocket(v.fd), &self.overlapped, &bytes_transferred, .FALSE, &flags);
...
pub const ShutdownError = posix.Error || error{
    Unexpected,
};
