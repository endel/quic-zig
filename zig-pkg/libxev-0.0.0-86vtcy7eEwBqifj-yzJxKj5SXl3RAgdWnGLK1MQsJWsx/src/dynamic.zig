...
pub fn Xev(comptime bes: []const AllBackend) type {
    ...
    return struct {
        ...
        pub fn Union(comptime field: []const []const u8) type {
            return dynamicpkg.Union(Backend, bes, field, false);
        }

        pub fn TaggedUnion(comptime field: []const []const u8) type {
            return dynamicpkg.Union(Backend, bes, field, true);
        }
...
/// Build a new exhaustive enum containing only the active backends for this
/// platform. Pre-0.16 Zig built this with `@Type`; 0.16 replaces that with
/// the `@Enum` builtin. The enum preserves the numeric tag values from
/// `AllBackend` so `subset`/`superset` stay identity conversions through
/// `@intFromEnum`/`@enumFromInt`.
fn EnumSubset(comptime T: type, comptime values: []const T) type {
    const TagInt = @typeInfo(T).@"enum".tag_type;
    comptime var names: [values.len][:0]const u8 = undefined;
    comptime var vals: [values.len]TagInt = undefined;
    inline for (values, 0..) |v, i| {
        names[i] = @tagName(v);
        vals[i] = @intFromEnum(v);
    }
    const names_const = names;
    const vals_const = vals;
    return @Enum(TagInt, .exhaustive, &names_const, &vals_const);
}

/// Resolve a backend implementation type (e.g. `IO_Uring.Async`) by walking
/// the given field path over `be.Api()`. The backend is always in the active
/// set here because `Union` below only calls this for candidate backends.
fn PickType(
...
/// backend by common field name. Example, for Async on Linux (candidates
/// `{io_uring, epoll}`): Union(Backend, bes, &.{"Async"}, true) produces:
///
///   union(Backend) {
...
///   }
///
/// Pre-0.16 built this dynamically via `@Type`. Zig 0.16 replaced `@Type`
/// with targeted builtins: we use `@Union` (paired with `@Enum` inside
/// `EnumSubset`) to stitch together one union field per candidate backend.
/// Inactive backends never appear as fields, so `inline else` over the tag
/// naturally visits only candidates — no `void` filtering required.
fn Union(
    comptime BackendTag: type,
    comptime bes: []const AllBackend,
    comptime field: []const []const u8,
    comptime tagged: bool,
) type {
    const Attr = std.builtin.Type.UnionField.Attributes;
    comptime var names: [bes.len][:0]const u8 = undefined;
    comptime var types: [bes.len]type = undefined;
    comptime var attrs: [bes.len]Attr = undefined;
    inline for (bes, 0..) |be, i| {
        names[i] = @tagName(be);
        types[i] = PickType(be, field);
        attrs[i] = .{};
    }
    const names_const = names;
    const types_const = types;
    const attrs_const = attrs;
    return @Union(
        .auto,
        if (tagged) BackendTag else null,
        &names_const,
        &types_const,
        &attrs_const,
    );
