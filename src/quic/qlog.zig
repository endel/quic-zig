...
pub const QlogWriter = struct {
    ...
    pub fn init(dir_path: []const u8, odcid: []const u8, is_server: bool) ?Self {
        ...
        // Build full path: {dir_path}/{odcid_hex}_{role}.sqlog
        var path_buf: [std.fs.max_path_bytes]u8 = undefined;
        var pos: usize = 0;
        if (dir_path.len + 1 + odcid.len * 2 + 20 >= path_buf.len) return null;
        @memcpy(path_buf[pos..][0..dir_path.len], dir_path);
        pos += dir_path.len;
        if (pos == 0 or path_buf[pos - 1] != '/') {
            path_buf[pos] = '/';
            pos += 1;
        }
        for (odcid) |b| {
    ...
    pub fn serializeSentFrames(pkt: *const ack_handler.SentPacket, out: []u8) usize {
        ...
        for (pkt.getStreamFrames()) |sf| {
            w.frame("stream");
            w.print(",\"stream_id\":{d},\"offset\":{d},\"length\":{d}", .{ sf.stream_id, sf.offset, sf.length });
            if (sf.fin) w.print(",\"fin\":true", .{});
            w.end();
        }
    ...
    const Writer = struct {
        ...
        fn frame(self: *Writer, name: []const u8) void {
            if (!self.first) self.print(",", .{});
            self.first = false;
            self.print("{{\"frame_type\":\"{s}\"", .{name});
        }
