...
pub const Config = struct {
    ...
    /// Number of worker threads. 0 = single-threaded (current behavior).
    /// When > 0, a recv thread dispatches packets to N workers by CID.
    workers: u16 = 0,
...
pub fn Server(comptime Handler: type) type {
    return struct {
        ...
        /// If workers > 0 in config, spawns multi-threaded workers instead.
        pub fn run(self: *Self) !void {
            self.start();
            try self.loop.run(.until_done);
        }

        /// Run with N worker threads. The main thread receives all packets
        /// and dispatches to workers by connection ID.
        pub fn runMulti(alloc: std.mem.Allocator, handler: *Handler, config: Config) !void {
            const num_workers = if (config.workers > 0) config.workers else 1;
            if (num_workers <= 1) {
                var server = try Self.init(alloc, handler, config);
                defer server.deinit();
                return server.run();
            }

            // Build shared TLS/conn config once
            const tls_config: tls13.TlsConfig = if (config.tls_config) |tc| tc else blk: {
                const server_cert_pem = try std.fs.cwd().readFileAlloc(alloc, config.cert_path, 8192);
                const server_key_pem = try std.fs.cwd().readFileAlloc(alloc, config.key_path, 8192);
                const cert_chain = try tls13.parsePemCertChain(alloc, server_cert_pem);
                ...
                break :blk .{
                    .cert_chain_der = cert_chain,
                    .private_key_bytes = try alloc.dupe(u8, ec_key),
                    .alpn = alpn,
                    .ticket_key = ticket_key,
                };
            };
            ...
            var conn_config_base: connection.ConnectionConfig = if (config.conn_config) |cc| cc else blk: {
                var cc: connection.ConnectionConfig = .{ .token_key = retry_token_key };
                if (Handler.protocol == .webtransport or Handler.protocol == .quic) {
                    cc.max_datagram_frame_size = config.max_datagram_frame_size;
                }
            ...
            const sockfd, const local_addr = if (config.ipv6) blk: {
                ...
                posix.setsockopt(fd6, posix.IPPROTO.IPV6, IPV6_V6ONLY, std.mem.asBytes(&@as(c_int, 0))) catch {};
                posix.setsockopt(fd6, posix.SOL.SOCKET, posix.SO.REUSEADDR, &std.mem.toBytes(@as(c_int, 1))) catch {};
                try posix.bind(fd6, &addr6.any, addr6.getOsSockLen());
                break :blk .{ fd6, addr6 };
            } else blk: {
            ...
            ecn_socket.enableEcnRecv(sockfd) catch {};
            const local_storage = connection.sockaddrToStorage(&local_addr.any);

            // Allocate worker queues and threads
            const queues = try alloc.alloc(WorkerQueue, num_workers);
            for (queues) |*q| q.* = .{};
            defer alloc.free(queues);

            var stop = std.atomic.Value(bool).init(false);

            const worker_ctxs = try alloc.alloc(WorkerCtx, num_workers);
            defer alloc.free(worker_ctxs);

            const threads = try alloc.alloc(std.Thread, num_workers);
            ...
            for (0..num_workers) |i| {
                worker_ctxs[i] = .{
                    ...
                    .stop_flag = &stop,
                    .tls_cfg = tls_config,
                    .conn_cfg = conn_config_base,
                    .retry_token_key = retry_token_key,
                    .static_key = static_reset_key,
                ...
                threads[i] = try std.Thread.spawn(.{}, workerThread, .{&worker_ctxs[i]});
            }

            std.log.info("QUIC server listening on {s}:{d} ({d} workers)", .{ config.address, config.port, num_workers });

            // Main recv loop: read packets and dispatch by CID
            var recv_buf: [8192]u8 = undefined;
            var next_worker: u32 = 0; // round-robin for Initial packets

            while (!stop.load(.acquire)) {
                const recv_result = ecn_socket.recvmsgEcn(sockfd, &recv_buf) catch |err| {
                ...
                // Determine worker by CID byte 0
                var worker_idx: u32 = next_worker;
                const pkt = recv_buf[0..recv_result.bytes_read];
                if (pkt.len > 0) {
                    const first_byte = pkt[0];
                    if (first_byte & 0x80 != 0) {
                        // Long header: DCID starts at byte 6 (after type+version+dcid_len)
                        if (pkt.len > 6) {
                            const dcid_len = pkt[5];
                            if (dcid_len > 0 and pkt.len > 6 + dcid_len) {
                                worker_idx = pkt[6] % @as(u8, @intCast(num_workers));
                            }
                        }
                        // Initial packets (type bits = 00 in long header) → round-robin
                        ...
                        if (pkt.len > 1) {
                            worker_idx = pkt[1] % @as(u8, @intCast(num_workers));
                        }
                    }
                }

                var qe: QueueEntry = .{};
                const copy_len = @min(recv_result.bytes_read, qe.data.len);
                @memcpy(qe.data[0..copy_len], pkt[0..copy_len]);
                qe.len = @intCast(copy_len);
                qe.from_addr = recv_result.from_addr;
                qe.addr_len = recv_result.addr_len;
                qe.ecn = recv_result.ecn;

                if (!queues[worker_idx].push(qe)) {
                    // Queue full, try next worker
                    const fallback = (worker_idx + 1) % num_workers;
                    _ = queues[fallback].push(qe);
                }
            }
        ...
        const QueueEntry = struct {
            ...
            len: u16 = 0,
            from_addr: posix.sockaddr.storage = undefined,
        ...
        const WorkerQueue = struct {
            ...
            fn pop(self: *@This()) ?*const QueueEntry {
                const head = self.head.load(.acquire);
                if (head == self.tail.load(.acquire)) return null;
                const entry = &self.entries[head];
                self.head.store((head + 1) % QUEUE_CAP, .release);
                return entry;
            }
        };
        ...
        fn workerThread(ctx: *WorkerCtx) void {
            ...
            defer arena.deinit();
            const w_alloc = arena.allocator();

            var worker_conn_cfg = ctx.conn_cfg;
            worker_conn_cfg.worker_id = ctx.worker_id;
            worker_conn_cfg.num_workers = ctx.num_workers;
            ...
            conn_mgr.require_retry = ctx.require_retry;
            defer conn_mgr.deinit();

            var out_buf: [1500]u8 = undefined;
            var batch = ecn_socket.SendBatch.init(ctx.sock);

            std.log.info("worker {d}: started", .{ctx.worker_id});
            ...
            while (!ctx.stop_flag.load(.acquire)) {
                ...
                while (ctx.queue.pop()) |entry| {
                    var pkt_copy: [1500]u8 = undefined;
                    @memcpy(pkt_copy[0..entry.len], entry.data[0..entry.len]);

                    switch (conn_mgr.recvDatagram(
                        pkt_copy[0..entry.len],
                        entry.from_addr,
                        ctx.local,
                        entry.ecn,
                        &out_buf,
                    )) {
                        .processed => |conn_entry| {
                            const conn = conn_entry.conn;
                            const bytes_written = conn.send(&out_buf) catch continue;
                            if (bytes_written > 0) {
                        ...
                        .send_response => |data| {
                            batch.add(data, @ptrCast(&entry.from_addr), entry.addr_len, 0);
                        },
                        .dropped => {},
                    }
                    processed += 1;
                ...
                if (processed > 0) {
                    ...
                    // Process protocol events (H3/WT init + dispatch)
                    for (conn_mgr.entries.items) |conn_entry| {
                        const conn = conn_entry.conn;
                        if (conn.isEstablished() and !conn_entry.h3_initialized) {
                            // Init H3 layer
                            conn_entry.h3_conn = h3.H3Connection.init(w_alloc, conn, true);
                            if (Handler.protocol == .h3) {
                                conn_entry.h3_conn.?.initConnection() catch {};
                            }
                            conn_entry.h3_initialized = true;
                        }

                        // Poll H3 events
                        if (conn_entry.h3_conn != null and Handler.protocol == .h3) {
                            var h3c = &conn_entry.h3_conn.?;
                            var session_obj = Session{ .entry = conn_entry };
                            while (true) {
                                ...
                                switch (event.?) {
                                    .headers => |hdr| {
                                        if (@hasDecl(Handler, "onRequest")) {
                                            ctx.handler_ptr.onRequest(&session_obj, hdr.stream_id, hdr.headers);
                                        }
                                    },
                                    .data => |d| {
                                        if (@hasDecl(Handler, "onData")) {
                                            ...
                                            while (true) {
                                                ...
                                                if (n == 0) break;
                                                ctx.handler_ptr.onData(&session_obj, d.stream_id, body_buf[0..n]);
                                            }
                                        } else {
                    ...
                    // Tick connections (timeouts, send bursts)
                    var i: usize = 0;
                    while (i < conn_mgr.entries.items.len) {
                        const conn_entry = conn_mgr.entries.items[i];
                        if (!conn_mgr.tickEntry(conn_entry)) {
                            continue;
                        }
                        const conn = conn_entry.conn;
                        var send_count: usize = 0;
                        while (send_count < 1000) : (send_count += 1) {
                            const bw = conn.send(&out_buf) catch break;
                            if (bw == 0) break;
                            const sa = conn.peerAddress();
                            batch.add(out_buf[0..bw], @ptrCast(sa), connection.sockaddrLen(sa), conn.getEcnMark());
                        }
                        i += 1;
                    }
                    batch.flush();
                } else {
                    // No packets — handle timeouts
                    for (conn_mgr.entries.items) |conn_entry| {
                        conn_entry.conn.onTimeout() catch {};
                    }
                    std.Thread.sleep(50 * std.time.ns_per_us);
                }
            }

            std.log.info("worker {d}: stopped", .{ctx.worker_id});
        }
