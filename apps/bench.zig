...
const ConnState = struct {
    ...
    request_times: [4096]i64 = undefined,
    done: bool = false,
...
fn runBench(alloc: std.mem.Allocator, config: BenchConfig) !void {
    ...
    var all_latencies = try alloc.alloc(i64, total_requests);
    ...
    // Run connections sequentially (each does N requests)
    var conn_idx: u32 = 0;
    while (conn_idx < config.num_connections) : (conn_idx += 1) {
        ...
        // === Requests ===
        var req_idx: u32 = 0;
        while (req_idx < config.requests_per_conn) : (req_idx += 1) {
            ...
            if (got_response) {
                const req_end = timestamp();
                const latency = req_end - req_start;
                if (latency_count < all_latencies.len) {
                    all_latencies[latency_count] = latency;
                    latency_count += 1;
                }
            }
        }
    ...
    const bench_duration_ns = bench_end - bench_start;
    const bench_duration_s: f64 = @as(f64, @floatFromInt(bench_duration_ns)) / 1_000_000_000.0;

    // === Results ===
    std.debug.print("\n", .{});
    std.debug.print("═══════════════════════════════════════════\n", .{});
    std.debug.print("  quic-zig benchmark results\n", .{});
    std.debug.print("═══════════════════════════════════════════\n", .{});
    ...
    if (latency_count > 0) {
        ...
        std.mem.sort(i64, all_latencies[0..latency_count], {}, std.sort.asc(i64));

        const p50 = all_latencies[latency_count / 2];
        const p99_idx = @min(latency_count - 1, latency_count * 99 / 100);
        const p99 = all_latencies[p99_idx];
        const p999_idx = @min(latency_count - 1, latency_count * 999 / 1000);
        const p999 = all_latencies[p999_idx];
        const min_lat = all_latencies[0];
        const max_lat = all_latencies[latency_count - 1];

        var sum: i128 = 0;
        for (all_latencies[0..latency_count]) |l| sum += l;
        const avg: f64 = @as(f64, @floatFromInt(sum)) / @as(f64, @floatFromInt(latency_count)) / 1000.0;

        const rps: f64 = @as(f64, @floatFromInt(latency_count)) / bench_duration_s;
        ...
        std.debug.print("    min:   {d:.0}\n", .{@as(f64, @floatFromInt(min_lat)) / 1000.0});
        ...
        std.debug.print("    max:   {d:.0}\n", .{@as(f64, @floatFromInt(max_lat)) / 1000.0});
    }
    std.debug.print("═══════════════════════════════════════════\n", .{});
}
...
pub fn main() !void {
    ...
    std.debug.print("quic-zig bench: {d} connections × {d} requests → 127.0.0.1:{d}\n", .{
        config.num_connections, config.requests_per_conn, config.port,
    });
