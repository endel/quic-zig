#!/bin/bash
# Run the QUIC Interop Runner with quic-zig
#
# Usage:
#   ./interop/runner/run.sh                          # handshake test, self-interop
#   ./interop/runner/run.sh handshake,transfer       # specific tests
#   ./interop/runner/run.sh handshake quic-go        # test against quic-go
#
# Prerequisites:
...
# Prefer Homebrew OpenSSL over macOS LibreSSL (certs.sh needs OpenSSL features)
...
docker build --platform linux/amd64 -t "$IMAGE_TAG" -f "$SCRIPT_DIR/Dockerfile" "$ROOT_DIR"

echo ""
echo "=== Injecting quic-zig into implementations_quic.json ==="

# Add quic-zig to the runner's implementations.json (idempotent)
$PYTHON -c "
import json
path = '$RUNNER_DIR/implementations_quic.json'
with open(path) as f:
    impls = json.load(f)
impls['quic-zig'] = {
    'image': '$IMAGE_TAG',
...
echo ""
echo "=== Running interop tests: $TESTS ==="

if [ "$PEER" = "quic-zig" ]; then
    echo "    quic-zig <-> quic-zig"
    $PYTHON run.py -s quic-zig -c quic-zig -t "$TESTS" -d
else
    # Pull peer image
